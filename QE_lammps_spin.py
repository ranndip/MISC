...
class QExpresso:
    ...
    def fixCellMat(self):
        ...
        alpha = self.vec2angle(a, b)
        beta = self.vec2angle(b, c)
        gamma = self.vec2angle(a, c)
        ...
        self.xlo_bound = min(0.0, self.xy, self.xz, self.xy + self.xz)
        self.xhi_bound = self.la + max(0.0, self.xy, self.xz, self.xy + self.xz)

        self.ylo_bound = min(0.0, self.yz)
        self.yhi_bound = self.lb + max(0.0, self.yz)

        self.zlo_bound = 0.0
        self.zhi_bound = self.lc
    ...
    def write(self, outFH, nFrames, iFrame):
        ...
        outFH.write("ITEM: NUMBER OF ATOM\n")
        outFH.write("%-10d\n" % self.nAtoms)
