...
class QExpresso:
    ...
    def readSpinSpiral(self):

        self.spinSpiral = dict()

        for lineID, line in enumerate(self.lines):
            if "atom number" in line and "relative position" in line:
                atomID = int(line.split()[2]) - 1
                spinLine = self.lines[lineID + 4]

                theta, phi = (float(a) for a in spinLine.split()[-2::])

                self.spinSpiral[atomID] = (theta, phi)

        for key, val in self.spinSpiral.items():
            print("atomID: %d   (theta,phi): (%.3f,%.3f)" % (key, val[0], val[1]))
    ...
    def write(self, outFH, nFrames, iFrame):
        ...
        outFH.write("ITEM: ATOMS id type x y z sx sy sz\n")

        degree2rad = np.pi / 180.0
        for i in range(self.nAtoms):
            sx = np.cos(self.spinSpiral[i][1] * degree2rad) * np.sin(self.spinSpiral[i][0] * degree2rad)
            sy = np.sin(self.spinSpiral[i][1] * degree2rad) * np.cos(self.spinSpiral[i][0] * degree2rad) 
            sz = np.cos(self.spinSpiral[i][0] * degree2rad)

            outFH.write(
                "%-5d  %-5d  %22.16f %22.16f %22.16f %4.1f %4.1f %4.1f\n"
                % (
                    i + 1,
                    self.types[i],
                    self.cartCoords[i, 0],
