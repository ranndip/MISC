...
#             self.magMoment.append(m)
    ...
    def fixCellMat(self):
        ...
        p_a = np.linalg.norm(a)
        p_b = np.linalg.norm(b)
        p_c = np.linalg.norm(c)
        lx = p_a
        ...
        self.cellMat_fixed[0, 0] = lx
        self.cellMat_fixed[0, 1] = 0.0
        self.cellMat_fixed[0, 2] = 0.0

        self.cellMat_fixed[1, 0] = p_xy
        self.cellMat_fixed[1, 1] = ly
        self.cellMat_fixed[1, 2] = 0.0

        self.cellMat_fixed[2, 0] = p_xz
        self.cellMat_fixed[2, 1] = p_yz
        self.cellMat_fixed[2, 2] = lz
        
        #print(self.cellMat_fixed)
        # convert t ocart coordinates
        self.cartCoords = self.crystalCoords*self.latParam*bohr2ang 
        self.crystCoords=np.matmul(self.cartCoords,np.linalg.inv(self.cellMat))
        #print(self.crystCoords)
        self.cartCoords2=np.matmul(self.crystCoords,self.cellMat_fixed)
        #print(self.cartCoords2)
        ...
        for i in range(self.nAtoms):
            ...
            outFH.write(
                ...
                % (
                    i + 1,
                    self.types[i],
                    self.cartCoords2[i, 0],
                    self.cartCoords2[i, 1],
                    self.cartCoords2[i, 2],
                )
            )
